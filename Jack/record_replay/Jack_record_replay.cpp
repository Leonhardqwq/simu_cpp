...
int main(int argc, char* argv[]) {
    if (argc != 3) {
        std::cerr << "usage: Jack_record_replay <batch.json> <results.csv>" << std::endl;
        return 1;
    }
    ...
    try {
        ...
        for (const auto& item : cases) {
            ...
            try {
                ...
                JackConfig config(config_json);
                int m_min = item.at("M_min").get<int>();
                if (m_min > config.M) config.M = m_min;

                double boom_rate = work(config, true);
                fout << boom_rate << ",\n";
            }
